...
class QuranRepository {
  // We use the corrected data from the Help folder's Suras List.txt
  static const List<String> _arabicNames = [
    "الفاتحة", "البقرة", "آل عمران", "النساء", "المائدة", "الأنعام", "الأعراف", "الأنفال", "التوبة", "يونس",
    "هود", "يوسف", "الرعد", "إبراهيم", "الحجر", "النحل", "الإسراء", "الكهف", "مريم", "طه",
    "الأنبياء", "الحج", "المؤمنون", "النّور", "الفرقان", "الشعراء", "النّمل", "القصص", "العنكبوت", "الرّوم",
  ...
  // Corrected verse counts from Help/Suras List.txt
  static const List<int> _versesCount = [
    7, 286, 200, 176, 120, 165, 206, 75, 129, 109, 123, 111, 43, 52, 99, 128,
    111, 110, 98, 135, 112, 78, 118, 64, 77, 227, 93, 88, 69, 60, 34, 30, 73,
    54, 45, 83, 182, 88, 75, 85, 54, 53, 89, 59, 37, 35, 38, 29, 18, 45, 60,
    49, 62, 55, 78, 96, 29, 22, 24, 13, 14, 11, 11, 18, 12, 12, 30, 52, 52,
    44, 28, 28, 20, 56, 40, 31, 50, 40, 46, 42, 29, 19, 36, 25, 22, 17, 19,
    26, 30, 20, 15, 21, 11, 8, 5, 19, 5, 8, 8, 11, 11, 8, 3, 9, 5, 4, 6, 3, 6,
    3, 5, 4, 5, 6
  ];
  ...
  List<SurahModel> getAllSurahs() {
    return List.generate(114, (index) {
      return SurahModel(
        index: index,
        arabicName: _arabicNames[index],
        englishName: _englishNames[index],
        versesCount: _versesCount[index],
      );
    });
  }

  Future<List<String>> getSurahVerses(int index) async {
    try {
      final String data = await rootBundle.loadString('assets/suras/${index + 1}.txt');
      final List<String> verses = data.trim().split('\n');
      return verses;
    } catch (e) {
      return ["Error loading verses."];
    }
  }
}
